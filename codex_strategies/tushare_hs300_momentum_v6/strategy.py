...
def _benchmark_risk_on(end_date) -> tuple[bool, dict[str, float]]:
    ...
    risk_on = latest > ma60 and ret20 > -0.02
    return risk_on, {"latest": latest, "ma60": ma60, "ret20": ret20}


def _select_targets(context) -> List[str]:
...
def _rebalance_targets(context, targets: List[str]) -> None:
...
def rebalance(context):
    ...
    if not risk_on:
        _exit_all(
            context,
            "基准趋势偏弱: latest=%.2f ma60=%.2f ret20=%.2f%%"
            % (
