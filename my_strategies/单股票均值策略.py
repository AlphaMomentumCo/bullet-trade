...
def market_open(context):
    ...
    security = g.security
    # 用上一交易日作为结束日期，避免取到未来数据（QMT/JQData 都兼容）
    ...
    if close_data is None or len(close_data) < 5:
        log.warning(f"{security} 历史数据不足，跳过当日交易")
        return

    # 取得过去五天的平均价格
    MA5 = close_data['close'].tail(5).mean()
    # 取得当前参考价格
    ...
    if (current_price < 1.01*MA5) and (cash > 0):
        # 记录这次买入
        log.info("价格低于均价 1%%, 买入 %s" % (security))
        # 用所有 cash 买入股票
        order_value(security, cash)
        
    # 如果上一时间点价格低于五天平均价, 则空仓卖出
    elif current_price > MA5 and position and position.closeable_amount > 0:
        # 记录这次卖出
        log.info("价格高于均价, 卖出 %s" % (security))
